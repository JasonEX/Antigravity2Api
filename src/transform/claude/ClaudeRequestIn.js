...
function transformClaudeRequestIn(claudeReq, projectId, options = {}) {
  ...
  const effectiveSignatureSegmentStartIndex =
    signatureSegmentStartIndex == null ? currentTurnStartIndex : Math.max(signatureSegmentStartIndex, currentTurnStartIndex);
  ...
  if (claudeReq.messages) {
    for (let msgIndex = 0; msgIndex < claudeReq.messages.length; msgIndex++) {
      ...
      const shouldForwardThoughtSignatures =
        ...
        (effectiveSignatureSegmentStartIndex == null || msgIndex >= effectiveSignatureSegmentStartIndex);
      let role = msg.role;
      if (role === "assistant") {
        role = "model";
      }
      ...
      if (Array.isArray(msg.content)) {
        for (const item of msg.content) {
          if (item.type === "text") {
            ...
            const isRequiredByToolResultTurn = !!toolUseId && requiredToolUseIdsForToolResultTurn.has(toolUseId);
            ...
            const isInSignatureSegment =
              effectiveSignatureSegmentStartIndex == null || msgIndex >= effectiveSignatureSegmentStartIndex;
            const shouldForwardToolSignature =
            ...
            if (mcpXmlEnabled && isMcpToolName(item?.name)) {
              ...
              const pendingSig = shouldForwardToolSignature ? pendingThoughtSignature : null;
              ...
              if (itemSig && shouldForwardToolSignature) {
                part.thoughtSignature = itemSig;
              } else if (pendingSig && shouldForwardToolSignature) {
            ...
            } else if (shouldForwardToolSignature) {
              sig = getToolThoughtSignature(item.id);
            }

            // 消费 pending：无论是否使用（例如 tool_use 已自带签名），都认为已到达“thinking 之后的第一个输出 part”。
            if (pendingSig) pendingThoughtSignature = null;
            if (sig && shouldForwardToolSignature && !fcPart.thoughtSignature) {
            ...
            } else if (shouldForwardToolSignature && isFirstToolUseInMessage) {
